...
module Sdust
  class App
    ...
    def sdust(sequence : String | IO::Memory)
      repeat_value = 0
      repeat_window = 0
      win_len = 0
      ...
      seq_bytes = normalize_sequence(sequence)
        ...
        if b < 4 # an A/C/G/T base
          ...
          if (cont_len >= WordLength) # we have seen a word
            ...
            win_len, repeat_window, repeat_value = \
               shift_window(curr_word, window, win_len, repeat_window, repeat_value, curr_window, curr_value)
            if (repeat_window * 10 > win_len * threshold)
              find_perfect(perfect_intervals, window, start, win_len, repeat_value, curr_value)
            end
          end
    ...
    def shift_window(
      curr_word : Int32, window : Array(Int32), win_len : Int32,
      repeat_window : Int32, repeat_value : Int32,
